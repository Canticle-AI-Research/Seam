...
- Append-only roadmap lifecycle: future possibilities, committed work, and
  observed outcomes live in separate `future-ideas`, `plans`, and `executed`
  streams. One stable `roadmap:*` item ID crosses streams; `origin` links a
  promotion while `supersedes` links a correction to the exact prior event in
  the same item/stream. Old events are immutable, including failures and
  abandoned work. `tools.streams.roadmap_lifecycle` rejects missing or
  cross-item targets, backwards violations, and forked live heads, then
  regenerates `.seam/streams/roadmap_workflow_state.md` and the global
  cross-index. See `docs/roadmap/APPEND_ONLY_ROADMAP.md`.

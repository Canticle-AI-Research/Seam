...
- Root-supplied agent orchestration is governed by
...
- DeepSeek parallel audit execution is documented in `docs/SOP_DEEPSEEK_PARALLEL_AUDIT_EXECUTION.md` (see HISTORY#205). That SOP remains the durable DeepSeek-specific audit handoff. Its former Codex non-agentic restriction is superseded by the root-supplied orchestration policy above; model-specific execution still cannot weaken root ownership, packet bounds, or closeout gates.

...
- Roadmap registration is single-source and its derived stream is not gated
  (HISTORY#647). `ROADMAP.md` `seam:item` markers are authored-canonical and
  `.seam/streams/roadmap/state.md` is derived, but `tools/streams/verify_streams.py`
  contains no roadmap check, so the `AGENTS.md` Session End instruction to rerun
  `tools.streams.roadmap_parser` after a `ROADMAP.md` change is enforced by no
  local gate and no required CI check. Until a drift check exists, any change
  adding or retiring a marker must rerun the parser in the same change.
  One research area carries one registered track: overlapping lanes proposed in
  separate branches are reconciled to a single marker before either merges,
  because two markers for one area produce competing priorities, competing stage
  numbering, and a textual conflict when both patches target the same position.
...
- Claude Code defense-in-depth: ignored `.claude/settings.local.json` may wire `tools/claude/preflight_protocol.sh` (PreToolUse Bash hook) and `tools/claude/session_start_brief.sh` (SessionStart hook) so Claude Code reproduces the same verify chain before invoking git and prints the AGENTS.md read order on session start. Only the portable memory pin in `.claude/settings.json` is tracked; each Claude Code operator wires their own machine. The Claude hook is belt-and-suspenders to the canonical git hook; the git hook is the protocol enforcement, the Claude hook is the early warning. Equivalent wiring for Codex, Gemini, and other agents is open follow-up work tracked in HISTORY; the canonical git hook covers them today even without per-agent wiring.
- Codex-only root-supplied agent orchestration is governed by
  `docs/SOP_AGENT_ORCHESTRATION.md`. The root loads canonical startup context
  once and supplies schema-valid bounded packets to four logical domain roles:
  context, delivery, assurance, and release. Delegated agents take the
  `AGENTS.md` fast path, read only packet-listed context, and return
  `MISSING_CONTEXT` instead of surveying the repository. The maximum hierarchy
  is root to domain orchestrator to specialist; specialists cannot delegate,
  and root alone integrates, stages, updates continuity, manages the PR, and
  makes the final evidence-admission decision.
- Codex project `SessionEnd` queues an idempotent, fingerprint-anchored,

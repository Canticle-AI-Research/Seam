...
- [ ] Dispatch **Package release** on `main` with the exact version already in
      `pyproject.toml`.
- [ ] Record the successful **Package release** workflow run ID; the guarded
      publication step downloads its immutable Actions artifact as the trusted
      byte source for the reviewed draft.
- [ ] Confirm the dispatching account matches the repository
...
- [ ] Dispatch **Publish reviewed private release** from current `main` with the
      exact version, tagged commit SHA, preparation run ID, reviewed manifest
      digest, and reviewed notes digest; affirm the immutability check only
      after the draft review. If `main` advanced during review, restart
      preparation from the new head.

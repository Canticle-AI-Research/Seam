...
---BEGIN-ENTRY-#347---
id: 347
date: 2026-07-03T23:54:27Z
agent: claude
status: done
topics: mcp, pyproject, readme, registry
commits: none
refs: pyproject.toml,README.md,server.json,seam_runtime/mcp_protocol.py
supersedes: none
tokens: 466
---
...
---BEGIN-ENTRY-#350---
id: 350
date: 2026-07-06T01:01:48Z
agent: claude
status: done
topics: mcp, registry, pypi, release
commits: none
refs: pyproject.toml,server.json,README.md,seam_runtime/mcp_protocol.py
supersedes: none
tokens: 652
---
Completed the MCP Server Registry publish that HISTORY#348 left pending on the operator's interactive login. Ran mcp-publisher publish (v1.7.9 binary, downloaded from the official modelcontextprotocol/registry GitHub release since it was not installed locally) after the operator completed 'mcp-publisher login github'; it failed with HTTP 403 ('You have permission to publish: io.github.BlackhatShiftey/*. Attempting to publish: io.github.blackhatshiftey/seam-runtime') -- the registry's GitHub-OAuth authorization is scoped to the exact-case GitHub login 'BlackhatShiftey', but server.json and README.md's ownership-verification comment had lowercased it to 'blackhatshiftey' since HISTORY#347 first wrote them. Fixed the casing in both files, but publish then failed a second time with a 400 (PyPI ownership check: 'must appear as mcp-name: io.github.BlackhatShiftey/seam-runtime in the package README') -- the registry validates PyPI package ownership against the README already baked into the published PyPI artifact, not the local file or GitHub repo, and the live v1.3.0 wheel/sdist on PyPI still carried the old lowercase comment (confirmed via a real GET to pypi.org's JSON API). PyPI files are immutable, so this required a real version bump: pyproject.toml, server.json (both top-level and package version fields), and seam_runtime/mcp_protocol.py's initialize serverInfo.version literal all bumped 1.3.0 -> 1.3.1. Rebuilt with 'python -m build', confirmed the corrected mcp-name casing inside the built wheel's METADATA before upload, then uploaded via twine (twine's default ~/.pypirc lookup was blocked by a stray pre-existing empty directory at that path from an earlier session -- worked around with --config-file /dev/null rather than touching the directory). Verified live: GET pypi.org/pypi/seam-runtime/1.3.1/json shows the corrected README. Re-ran mcp-publisher validate (clean) and publish -- succeeded this time. Verified end-to-end against the real registry API (GET registry.modelcontextprotocol.io/v0/servers?search=seam-runtime): io.github.BlackhatShiftey/seam-runtime v1.3.1 is live with status=active, isLatest=true. seam-runtime is now discoverable and installable (uvx --from seam-runtime seam-mcp) via the official MCP Server Registry for the first time. Verification: tests/audit/test_github_package_metadata.py, test_mcp_error_sanitization.py, test_mcp_artifact_containment.py, test_mcp_line_cap.py, test_mcp_tools_call_smoke.py, and test_mcp_stdio_smoke.py all pass post-bump. Full suite not yet re-run as of this entry -- queued next. Operator note: the stray empty ~/.pypirc directory on this machine was left in place (not deleted) since its origin is unclear and it is outside the repo; flagging for the operator to clean up or investigate at their discretion.
---END-ENTRY-#350---

---BEGIN-ENTRY-#351---
id: 351
date: 2026-07-06T01:12:39Z
agent: claude
status: changed
topics: protocol, git-hooks, security
commits: none
...
---BEGIN-ENTRY-#355---
id: 355
date: 2026-07-06T03:16:22Z
agent: claude
status: changed
topics: protocol, security, release, routing
commits: none
refs: tools/release/public_manifest.py,tools/release/sync_public_mirror.py,tools/release/public_seed/,tools/release/verify_public_safe.py,docs/PROTECTION_MODEL.md,REPO_LEDGER.md,docs/CODE_LAYOUT.md,seam_runtime/improvement.py
supersedes: none
tokens: 1159
---
Built a fail-closed public/private router for the seam-runtime mirror, replacing the deny-list-only gate from HISTORY#344. Root finding: the mirror sync (Canticle.cc release/package_release.py's push_repo_mirror -> 'git push seam-runtime main:main') mirrors the FULL private commit history and FULL tree; the only gate was verify_public_safe.py's secret/path deny-list, which fails open -- any private file that isn't secret-shaped ships anyway. Confirmed via git ls-tree on the real seam-runtime/main ref: HISTORY.md, HISTORY_INDEX.md, PROJECT_STATUS.md, REPO_LEDGER.md, .seam/, docs/audits/ (internal research), docs/SOP_*.md, docs/handoffs/, docs/roadmap/ (including COMPETITIVE_ROADMAP.md), docs/engineering/, docs/progress_tables/, docs/prompts/, .opencode/skills/, skills/, archive/, benchmarks/runs/ (raw experiment logs) have all been mirrored to the public repo since its creation -- not just current files but the entire git history (mirror is a strict ancestor of private main, confirmed via rev-list --left-right --count = '5 0', zero divergence). MANIFEST.in was checked separately and is NOT a second leak surface -- it's already a clean, narrow allow-list for the PyPI sdist.\n\nOperator decision (explicit): do not retroactively purge the already-public history (that requires filter-repo + force-push on the public repo, same class of irreversible operation as the earlier seam.db purge, deferred pending separate review) -- fix future syncs only. Also decided: the public repo should carry its OWN independent SEAM-protocol bookkeeping (fresh HISTORY.md/PROJECT_STATUS.md/REPO_LEDGER.md/HISTORY_INDEX.md), not a stripped copy of this repo's internal ones, since README.md/CONTRIBUTING.md already present AGENTS.md's protocol as a reusable pattern for contributors.\n\nBuilt: tools/release/public_manifest.py (fail-closed allow-list: is_public_synced_path for verbatim-copied paths -- seam_runtime/, tests/, installers/, tools/h2+history+streams/, curated docs/ subset, benchmarks/external+fixtures+fidelity+registry/, top-level policy/config files; is_public_owned_path for .seam/ and the four bookkeeping files the public repo maintains independently). tools/release/public_seed/ holds one-time seed templates for the owned paths, generated by actually running tools.history.new_entry/write_snapshot and tools.streams.history_adapter/rebuild_cross_index against a throwaway temp repo rather than hand-crafting the HISTORY.md entry format. tools/release/sync_public_mirror.py builds ONE new commit on top of the mirror's current tip (fast-forward, no history rewrite) via git plumbing (temp GIT_INDEX_FILE, write-tree, commit-tree) -- no working tree or local index touched; --push required to actually push, default is a dry-run diffstat. Hit and fixed one real bug during development: a freshly-created-but-unlinked-late tempfile path caused 'index file smaller than expected' from git's index parser -- fixed by unlinking the placeholder before use so update-index starts a fresh index. Upgraded tools/release/verify_public_safe.py to also enforce the allow-list (fail closed) as a backstop against any push that bypasses sync_public_mirror.py (e.g. a raw git push). Updated Canticle.cc's release/package_release.py push_repo_mirror() to call the new sync script instead of the raw full-tree push. Updated docs/PROTECTION_MODEL.md (new 'Public mirror sync mechanism' section), REPO_LEDGER.md, and docs/CODE_LAYOUT.md.\n\nSeparately found and fixed (operator-approved, same session): seam_runtime/improvement.py had an unguarded top-level 'from tools.h2.holdout_split import ...' that crashed on a real PyPI/uvx install (confirmed: ModuleNotFoundError). Every sibling usage of this exact tools/-not-shipped-in-the-wheel pattern (doctor.py, cli.py) already guards it with a try/except ModuleNotFoundError + source-checkout sys.path fallback; this file just missed the pattern. Fixed to match, with a clear RuntimeError when tools/h2 genuinely isn't available (wheel install) instead of crashing at import time.\n\nDry-run verified against the REAL private repo and REAL seam-runtime remote (fetched, not just simulated): 169 files changed, 639 insertions(+), 29654 deletions(-) vs the mirror's current tip -- confirms the router strips exactly the internal-bookkeeping bloat identified above while leaving HISTORY.md/PROJECT_STATUS.md/REPO_LEDGER.md/HISTORY_INDEX.md/.seam/ untouched (already present on the mirror, correctly preserved as owned). NOT yet pushed (--push not invoked) -- the actual live sync to the public seam-runtime remote is deferred to a separate operator go-ahead per this session's risk-of-external-action discipline.\n\nVerification: new tests/audit/test_public_manifest.py (43 cases) and tests/audit/test_sync_public_mirror.py (3 cases, real throwaway-git-repo coverage including the owned-path-preservation and first-sync-seeding behaviors) added. tests/audit/test_public_safe_gate.py updated (2 existing tests used non-allow-listed synthetic paths that would now spuriously block; retargeted to allow-listed paths so they test content-detection, not allow-list behavior; added test_path_not_on_allow_list_blocks and test_allow_listed_paths_pass). Full suite: pytest tests/ -q -m 'not external' -> all passed, 2 pre-existing xfail, zero failures.
---END-ENTRY-#355---

---BEGIN-ENTRY-#356---
id: 356
date: 2026-07-06T12:37:09Z
agent: claude
status: done
topics: protocol, security, release
commits: none
refs: tools/release/sync_public_mirror.py
supersedes: 355
tokens: 422
---

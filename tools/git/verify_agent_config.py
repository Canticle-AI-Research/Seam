...
def verify(repo: Path, *, staged: bool = False) -> list[str]:
    ...
    for entry in filter(None, entries):
        ...
        if path == PIN_PATH:
            ...
            try:
                content = (
                    subprocess.check_output(["git", "show", f":{path}"], cwd=repo)
                    if staged
                    else (repo / path).read_bytes()
                )
                valid = validate_pin(content, mode)
                if not staged and (repo / path).is_symlink():
                    valid = False
            except (OSError, subprocess.CalledProcessError):
                valid = False
            if not valid:
                problems.append(f"{path}: only the fixed autoMemoryDirectory pin is allowed")
        elif ".claude" in Path(path).parts or (path in changed and (
    ...
    if not saw_pin:
        path = repo / PIN_PATH
        if staged or not path.is_file() or path.is_symlink() or not validate_pin(path.read_bytes()):
